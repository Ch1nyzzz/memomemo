...
\texttt{best\_iterations}-tagged dirs (NA for progressive, which
does not surface a best list). Workspace and unique-read columns
...
effectively ignores \texttt{worst}. By contrast, three default+direction
...
\begin{figure}[t]
  ...
  same rate as an unmarked ref. Under default+direction, with no
...
a $14$--$22\times$ share of the proposer's attention; the same
fixed inspection budget that default-family spends on a
recency-biased tail is redirected by \name onto historically
productive iteration-and-file targets. The policy's labels, not
the budget tier itself, drive the proposer's bounded attention.
